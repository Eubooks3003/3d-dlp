...
from .gui_reset import GUIReset

from .gui_3d import O3DPointCloudViewer


# COLORS = ['red', 'blue', 'yellow', 'green', 'purple', 'orange']

class GUI(GUIUpdate, GUISelect, GUIReset):
    def __init__(self, root, canvas_size=500):
        ...
        self.available_models = [d for d in os.listdir('./checkpoints') 
        if ('dlp' in d and os.path.isdir(f'./checkpoints/{d}') 
        and len(os.listdir(f'./checkpoints/{d}')) > 0)] 
        ...
        self.o3d_viewer = O3DPointCloudViewer(title="3D-DLP: Scene Point Cloud")
        self.o3d_viewer.start()
    ...
    def on_generate_press(self):
        ...
        self.reset_btn = ttk.Button(root, text="Reset", command=self.reset, style='my.TButton')
    ...
    def get_particles(self):
        with torch.no_grad():
            if 'ddlp' in self.model_type:
                ...
                # --- RGB as 0..255 then scale like in training ---
                x = self.x_prepped
                print("X prepped shape: ", x.shape)
                if x is None:
                    raise RuntimeError("x_prepped not set. Call on_example_select_depth(...) first.")
                particle_dict = self.model(x, deterministic=True)
                print("PARTICLE DICT: ", particle_dict.keys())
