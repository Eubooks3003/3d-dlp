...
from modules.vae_modules import DVaeContext, DVaeEncoder, DVaeDecoder, DVaeDynamics
# util functions
...
class DVae(nn.Module):
    def __init__(self,
                 ...
                 res_from_fc=8,  # Initial resolution for background encoder-decoder
                 ch_mult=(1, 2, 3),  # Channel multipliers for background encoder-decoder
                 base_ch=32,  # Base channels for background decoder
                 final_cnn_ch=32,  # Final CNN channels for background encoder-decoder
                 ...
                 init_conv_std=0.02,  # std for conv bg normal dist (<fg -> prioritize fg in learning)
                 ):
        ...
        res_from_fc (int): Initial resolution for background encoder-decoder. Defaults to 8.
        ch_mult (tuple): Channel multipliers for background encoder-decoder. Defaults to (1, 2, 3).
        base_ch (int): Base channels for background encoder-decoder. Defaults to 32.
        final_cnn_ch (int): Final CNN channels for background encoder-decoder. Defaults to 32.
        ...
        self.encoder_module = DVaeEncoder(cdim=self.cdim,
                                         ...
                                         init_conv_std=init_conv_std,  # std for conv fg normal dist
                                         )

        self.n_kp_enc = self.n_kp_dec = self.encoder_module.n_kp_enc
        max_particles = self.n_kp_enc + 1  # particle positional bias, +1 for the bg particle, +1 for context

        # decoder
        self.decoder_module = DVaeDecoder(cdim=cdim, image_size=image_size,
                                         learned_feature_dim=self.learned_feature_dim,
                                          n_kp_enc=self.n_kp_dec, pad_mode=pad_mode,
                                         context_dim=self.context_dim,
        ...
        if self.context_dim > 0:
            self.ctx_module = DVaeContext(n_kp_enc=self.n_kp_enc, dropout=pte_dropout,
                                         learned_feature_dim=self.learned_feature_dim,
        ...
        if self.is_dynamics_model:
            ...
            self.dyn_module = DVaeDynamics(self.learned_feature_dim,
                                          pint_inner_dim,
                                          pint_inner_dim,
    ...
    def init_weights(self):
        ...
        if isinstance(self.ctx_module, DVaeContext):
            self.ctx_module.init_weights()
        if isinstance(self.dyn_module, DVaeDynamics):
    ...
    def info(self):
        ...
        cnn_info = [
            ("CNN Output Shape", self.encoder_module.encoder.cnn_out_shape),
            ("Decoder Background Upsamples", self.decoder_module.num_upsample)
        ]
        ...
        context_coeff = 1 if self.ctx_pool_mode != 'none' else (self.n_kp_enc)
        latent_dim = ((self.learned_feature_dim) * self.n_kp_enc
                      + context_coeff * self.context_dim)
        sections.append(format_row("Encoder", self.encoder_module.encoder.info))
        ...
        sections.append(format_row("Decoder", self.decoder_module.dec.info))

        # Add latent dimension with formula
        latent_formula = (f"({self.learned_feature_dim}) * {self.n_kp_enc} + "
    ...
    def decode_all(self, z_features, z_ctx, warmup=False):
        dec_dict = self.decoder_module(z_features, z_ctx, warmup)
        rec = dec_dict['rec']
    ...
    def sample_from_x(self, x, num_steps=10, deterministic=True, cond_steps=None, return_z=False, use_all_ctx=False,
        ...
        if decode:
            if z_context is not None:
                ...
                z_ctx = None
            dec_dict = self.decode_all(z_features, z_ctx=z_ctx)
        ...
        dyn_out = self.dyn_module.sample(z_features, z_context,
                                         steps=num_steps, deterministic=deterministic, actions=actions_dyn,
        ...
        if return_z:
            z_ids = 1 + torch.arange(z_features_dyn.shape[2], device=z_features_dyn.device)  # num_particles, ids start from 1
            z_ids = z_ids[None, None, :].repeat(z_features_dyn.shape[0], z_features_dyn.shape[1], 1)  # [bs, T, n_particles]
            z_out = {'z_features': z_features_dyn.detach(),
                     'z_context': z_context_dyn.detach(), 'z_ids': z_ids,
        ...
        if decode:
            dec_out = self.decode_all(z_features_dyn, z_context_dyn)
    ...
    def sample_from_z(self, z_features, z_context,
                      num_steps=10, deterministic=True, decode=False, actions=None, actions_mask=None,
                      lang_embed=None, z_goal=None):
        assert self.is_dynamics_model, f"model timesteps: {self.timestep_horizon} -> non-dynamics model"
        batch_size = z_features.shape[0]
        # dynamics
        dyn_out = self.dyn_module.sample(z_features, z_context,
                                         steps=num_steps, deterministic=deterministic, actions=actions,
    ...
    def forward(self, x, deterministic=False, warmup=False, with_loss=False, beta_kl=0.1, beta_dyn=0.1,
        ...
        dec_dict = self.decode_all(z_features, z_context, warmup)
        ...
        if self.is_dynamics_model:
            ...
            if actions_mask is not None:
                ...
                actions_mask_v = None
            dyn_out = self.dyn_module(z_features_v,
                                      z_context_v,
                                      actions=actions_v,
        ...
        output_dict = {'rec': rec, 'rec_rgb': rec_rgb,
                       ...
                       'mu_features_dyn': mu_features_dyn,
                       'logvar_features_dyn': logvar_features_dyn,
        ...
        if with_loss:
            ...
            loss_dict = self.calc_elbo(x, output_dict, warmup=warmup, beta_kl=beta_kl,
                                       ...
                                       num_static=num_static, done_mask=done_mask)
    ...
    def calc_dyn_elbo(self, x, model_output, warmup=False, beta_kl=0.1, beta_dyn=0.1, beta_rec=1.0,
                      ...
                      balance=0.5, beta_dyn_rec=1.0, num_static=1,
                       done_mask=None):
        ...
        if self.features_dist == 'categorical':
            ...
            loss_kl_feat_obj = loss_kl_feat_obj.view(batch_size, -1, mu_features.shape[2])
            # loss_kl_feat_obj = loss_kl_feat_obj.sum(dim=(-2, -1)).mean()
            loss_kl_feat_obj = (loss_kl_feat_obj.sum(-1) * done_mask_0).sum(-1).mean()

            loss_kl_feat = loss_kl_feat_obj
        else:
            loss_kl_feat = calc_kl(logvar_features_0.reshape(-1, logvar_features_0.shape[-1]),
                                   ...
                                   reduce='none')
            loss_kl_feat_obj = loss_kl_feat.view(batch_size, -1, mu_features.shape[2])
            # loss_kl_feat_obj = loss_kl_feat_obj.sum(dim=(-2, -1)).mean()
            loss_kl_feat_obj = ((loss_kl_feat_obj.sum(-1) * done_mask_0).sum(-1)).mean()
            loss_kl_feat = loss_kl_feat_obj
        ...
        features_weight = 1.0
        loss_kl_dyn_dict = calc_dynamic_kl(mu_features_post=mu_features_dyn_post,
                                           ...
                                           n_fg_classes=self.n_fg_classes,)
        ...
        loss_kl_dyn = ((loss_kl_dyn * discount[None, :]) * done_mask_dyn).sum(-1).mean()
        ...
        loss_kl_context_dyn = (loss_kl_context_dyn * done_mask_dyn).sum(-1).mean()
        ...
        if self.global_ctx_pool and mu_context_global is not None:
            ...
            loss_kl_context_global_dyn = loss_kl_context_global_dyn.view(batch_size,
                ...
                -1)
            loss_kl_context_global_dyn = (loss_kl_context_global_dyn * done_mask_dyn).sum(
                -1).mean()
        ...
        # total losses
        loss_kl = kl_balance * loss_kl_feat
    ...
    def calc_static_elbo(self, x, model_output, warmup=False, beta_kl=0.05, beta_dyn=1.0, beta_rec=1.0,
        ...
        if self.features_dist == 'categorical':
            ...
            loss_kl_feat_obj = loss_kl_feat_obj.view(-1, mu_features.shape[2])
            ...
            loss_kl_feat_obj = loss_kl_feat.view(-1, mu_features.shape[2])
            loss_kl_feat_obj = (loss_kl_feat_obj.sum(-1)).mean()
...
@torch.jit.script
def calc_dynamic_kl(mu_features_post, logvar_features_post, mu_features_prior, logvar_features_prior,
    ...
    bs = mu_features_post.shape[0]
    n_particles = mu_features_post.shape[1]
    ...
    if features_dist == 'categorical':
        ...
        loss_kl_features = (loss_kl_features.view(bs, n_particles)).sum(-1)
        ...
        loss_kl_features = (loss_kl_features.view(bs, n_particles)).sum(-1)
    # [bs, ]
    loss_kl_all = features_weight * loss_kl_features
...
if __name__ == '__main__':
    ...
    model_output = model(x, deterministic, warmup, with_loss=True, beta_kl=beta_kl,
                         ...
                         lang_embed=lang_demo, done_mask=ep_done_mask, x_goal=x_goal)
    ...
    mu_context: torch.Size([4, 11, 64, 7])
    logvar_context: torch.Size([4, 11, 64, 7])
    z_context: torch.Size([4, 11, 64, 7])
    mu_features_dyn: torch.Size([4, 10, 64, 4])
    logvar_features_dyn: torch.Size([4, 10, 64, 4])
    mu_context_dyn: torch.Size([4, 10, 64, 7])
    logvar_context_dyn: torch.Size([4, 10, 64, 7])
    ...
    all_losses = model.calc_elbo(x, model_output, beta_kl=beta_kl,
                                 beta_rec=beta_rec, kl_balance=kl_balance, beta_dyn=beta_dyn,
                                 num_static=num_static_frames,
                                 recon_loss_type="mse", warmup=warmup, done_mask=ep_done_mask)
    # let's see what's inside
    ...
    loss: 41.02410888671875
    psnr: 10.635334014892578
    kl: 0.2366456687450409
    kl_dyn: 1400.0897216796875
    loss_rec: 266.5931396484375
    loss_kl_feat: 236.64566040039062
    loss_kl_context: 36.153377532958984
    loss_kl_feat_dyn: 16361.103515625
    """
    ...
    if timestep_horizon > 1:
        ...
        z_context: torch.Size([1, 19, 64, 7])
        z_ids: torch.Size([1, 20, 64])
        z_context_posterior: torch.Size([1, 4, 64, 7])
        mu_context_posterior: torch.Size([1, 4, 64, 7])
        ...
        if action_cond:
            ...
            actions_demo = None
            actions_mask_demo = None
        z_out, rec_dyn = model.sample_from_z(z_features, z_context, num_steps=num_steps, deterministic=True,
        ...
        z_context: torch.Size([1, 34, 64, 7])
        ...
        z_context: torch.Size([1, 27, 64, 7])
        z_ids: torch.Size([1, 28, 64])
        z_context_posterior: torch.Size([1, 27, 64, 7])
        mu_context_posterior: torch.Size([1, 27, 64, 7])
