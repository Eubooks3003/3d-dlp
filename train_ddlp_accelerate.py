    ...
    for epoch in range(start_epoch, num_epochs):
        ...
        if epoch % eval_epoch_freq == 0 or epoch == num_epochs - 1:
            if accelerator.is_main_process:
                ...
                # if dec_objects_original.shape[1] != cropped_objects_original.shape[1]:
                #     print(cropped_objects_original.shape, dec_objects_original.shape)
                #     logvar_topk = torch.topk(logvar_sum, k=dec_objects_original.shape[1], dim=-1, largest=False)
                #     indices = logvar_topk[1]  # [batch_size, topk]
                #     batch_indices = torch.arange(dec_objects_original.shape[0]).view(-1, 1).to(dec_objects_original.device)
                #     cropped_objects_original = cropped_objects_original[batch_indices, indices]
                #     print(cropped_objects_original.shape, dec_objects_original.shape)
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="DDLP Multi-GPU Training")
