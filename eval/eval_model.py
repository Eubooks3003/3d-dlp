...
def evaluate_validation_dvae_elbo(model, config, epoch, batch_size=100, recon_loss_type="vgg", device=torch.device('cpu'),
                             save_image=False, fig_dir='./', recon_loss_func=None, beta_rec=1.0, beta_kl=1.0,
                             kl_balance=0.001, accelerator=None):
    model.eval()
    ds = config['ds']
    ch = config['ch']  # image channels
    image_size = config['image_size']
...
def evaluate_validation_dvae_elbo_dyn(model, config, epoch, batch_size=100, recon_loss_type="vgg",
                                 device=torch.device('cpu'),
                                 save_image=False, fig_dir='./', recon_loss_func=None, beta_rec=1.0,
                                 beta_kl=1.0, beta_dyn=1.0, beta_dyn_rec=1.0,
                                 kl_balance=1.0, accelerator=None, timestep_horizon=10,
    ...
    for batch in dataloader:
        ...
        with torch.no_grad():
            model_output = model(x, actions=actions, lang_embed=lang_embed, with_loss=True, beta_kl=beta_kl,
                                 beta_dyn=beta_dyn, beta_rec=beta_rec, kl_balance=kl_balance,
                                 recon_loss_type=recon_loss_type, recon_loss_func=recon_loss_func,
                                 beta_dyn_rec=beta_dyn_rec, done_mask=ep_done_mask, x_goal=x_goal)
        all_losses = model_output['loss_dict']
        loss = all_losses['loss']
