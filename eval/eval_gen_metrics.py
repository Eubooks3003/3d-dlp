...
def load_dvae_from_config(conf_path, ckpt_path=None):
    # load config
    try:
        config = get_config(conf_path)
    except FileNotFoundError:
    ...
    if ckpt_path is not None:
        try:
            model.load_state_dict(torch.load(ckpt_path, map_location=torch.device('cpu'), weights_only=False))
            print("loaded dvae model from checkpoint")
        except:
            print("dvae model checkpoint not found")
...
if __name__ == '__main__':
    ...
    parser.add_argument("--model_type", type=str, default='ddlp',
                        help="dataset to use: ['dlp', 'ddlp' 'dvae']")
    ...
    # load model config
    if model_type == 'dvae':
        pref = model_type = 'dvae'
    elif model_type == 'dlp' or model_type == 'gdlp':
    ...
    if model_type == 'dvae':
        model = load_dvae_from_config(conf_path, ckpt_path)
    else:
    ...
    if model_type == 'dlp':
        ...
        results = eval_ddlp_im_metric(model, device, timestep_horizon=timestep_horizon, val_mode=val_mode,
                                      config=config,
