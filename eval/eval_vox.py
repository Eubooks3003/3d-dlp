...
def log_vox_plotly_overlay(
    name,
    rec_vol, gt_vol,               # [C,D,H,W] or [D,H,W]
    step=None,
    channel=0,
    rec_mode="isosurface", gt_mode="isosurface",   # "volume" | "isosurface" | "points"
    rec_iso=0.5, gt_iso=0.5,
    rec_opacity=0.20, gt_opacity=0.20,             # used for "volume"
    rec_cmap="Oranges", gt_cmap="Blues",
    kps=None, kps_size=3
):
    Vrec = _pick_channel(rec_vol, channel)
    Vgt  = _pick_channel(gt_vol,  channel)
    D,H,W = Vrec.shape
    fig = go.Figure()

    def add_trace(fig, V, mode, iso, opacity, cmap, name):
        if mode == "volume":
            fig.add_trace(go.Volume(
                value=V.flatten(),
                x=np.repeat(np.arange(W), D*H),
            ...
            fig.add_trace(go.Isosurface(
                ...
                y=np.tile(np.repeat(np.arange(H), W), D),
                z=np.repeat(np.arange(D), H*W),
                isomin=iso, isomax=iso, surface_count=1,
                caps=dict(x_show=False, y_show=False, z_show=False),
                showscale=False, colorscale=cmap, name=name
            ))
        elif mode == "points":
            idx = np.argwhere(V >= iso)
            if idx.shape[0] > 120_000:
                sel = np.random.choice(idx.shape[0], 120_000, replace=False)
                idx = idx[sel]
            fig.add_trace(go.Scatter3d(
                x=idx[:,2], y=idx[:,1], z=idx[:,0],
                mode="markers",
                marker=dict(size=2, opacity=0.85),
                name=name
            ))

    add_trace(fig, Vgt,  gt_mode,  gt_iso,  gt_opacity,  gt_cmap,  "GT")
    add_trace(fig, Vrec, rec_mode, rec_iso, rec_opacity, rec_cmap, "REC")

    K = _kp_np(kps)

    if K is not None:
        K_ijk = _denorm_kp_to_index(K, D,H,W)
        fig.add_trace(go.Scatter3d(
            x=K_ijk[:,0], y=K_ijk[:,1], z=K_ijk[:,2],
            mode="markers",
            marker=dict(size=kps_size, symbol="x", color="red"),
