...
class GUI:
    def __init__(self, root, canvas_size=500):
        ...
        self.available_models = [d for d in os.listdir('./checkpoints') if
                                 ('dlp' in d and os.path.isdir(f'./checkpoints/{d}') and len(
                                     os.listdir(f'./checkpoints/{d}')) > 0)]
        ...
        self.ctxs = self.original_ctxs = None
    ...
    def on_example_select(self, example_dir):
        ...
        if self.model_type == 'dlp':
            kp = particle_dict['z'][0, 0]  # [n_kp, 2], [-1, 1]
            kp = self.normalize_kp(kp, normalize=False)
            kp = kp.cpu().numpy()

            scales = particle_dict['z_scale'][0, 0].cpu().numpy()
            depths = particle_dict['z_depth'][0, 0].cpu().numpy()
            features = particle_dict['z_features'][0, 0].cpu().numpy()
            obj_ons = particle_dict['obj_on'][0, 0].squeeze(-1).cpu().numpy()
            bg = particle_dict['z_bg_features'][0, 0].cpu().numpy()
            ctx = particle_dict['z_context'][0, 0].cpu().numpy()

            # glimpses = particle_dict['dec_objects_original']
            glimpses = particle_dict['dec_objects_original_rgb']
            alpha, rgb = torch.split(glimpses, [1, glimpses.shape[2] - 1], dim=2)
            rgb = rgb[:, :, :3]
            ...
            obj_ons = particle_dict['obj_on'].squeeze(-1).permute(1, 0).cpu().numpy()
            bg = particle_dict['z_bg_features'].cpu().numpy()  # [T, f]
            ctx = particle_dict['z_context'].cpu().numpy()
        ...
        self.original_bg = bg
        self.ctxs = self.original_ctxs = ctx
        # Add keypoints
        if self.model_type == 'dlp':
    ...
    def load_model(self):
        ...
        patch_channels = config['prior_channels']
        ...
        if self.model_type == 'ddlp' or self.model_type == 'diffuse_ddlp':
            model_type = 'ddlp'
            self.model = DLP(cdim=ch, enc_channels=enc_channels, patch_channels=prior_channels,
                             ...
                             filtering_heuristic=filtering_heuristic, context_dim=context_dim).to(
            ...
            #                  anchor_s=config['anchor_s'],
            #                  use_resblock=config['use_resblock'],
            #                  scale_std=config['scale_std'], timestep_horizon=timestep_horizon, context_dim=context_dim,
            #                  offset_std=config['offset_std'], obj_on_alpha=config['obj_on_alpha'],
            #                  obj_on_beta=config['obj_on_beta'], filtering_heuristic=filtering_heuristic).to(
            #     torch.device(f'{self.device_name}'))
            self.model = DLP(cdim=ch, enc_channels=enc_channels, patch_channels=patch_channels, image_size=image_size,
                             n_kp=n_kp,
                             pad_mode=pad_mode, dropout=0.0,
                             patch_size=patch_size, n_kp_enc=n_kp_enc, n_kp_prior=n_kp_prior,
                             learned_feature_dim=learned_feature_dim, learned_bg_feature_dim=learned_bg_feature_dim,
                             context_dim=context_dim,
                             kp_range=kp_range, kp_activation=kp_activation, anchor_s=anchor_s, predict_delta=True,
                             timestep_horizon=1,
                             use_resblock=use_resblock, scale_std=scale_std, offset_std=offset_std,
                             obj_on_alpha=obj_on_alpha,
                             obj_on_beta=obj_on_beta,
                             pint_layers=pint_layers,
                             pint_heads=pint_heads, pint_dim=pint_dim,
                             filtering_heuristic='none', attn_norm_type='rms', obj_res_from_fc=obj_res_from_fc,
                             obj_ch_mult=obj_ch_mult,
                             obj_base_ch=obj_base_ch, obj_final_cnn_ch=obj_final_cnn_ch, bg_res_from_fc=bg_res_from_fc,
                             bg_ch_mult=bg_ch_mult, bg_base_ch=bg_base_ch, bg_final_cnn_ch=bg_final_cnn_ch,
                             num_res_blocks=num_res_blocks).to(
                torch.device(f'{self.device_name}'))
    ...
    def decode_particles(self, kp=None, scales=None, depths=None, obj_ons=None, features=None, bg=None, ctx=None):
        if kp is None:
        ...
        z_bg = torch.tensor(bg, device=torch.device(self.device_name), dtype=torch.float).reshape(self.n_frames,
                                                                                                  self.original_bg.shape[
                                                                                                      -1])  # [T, F]
        z_context = torch.tensor(ctx, device=torch.device(self.device_name), dtype=torch.float).reshape(self.n_frames,
                                                                                                        self.original_ctxs.shape[
                                                                                                            -1])  # [T, F]
        decoder_dict = self.model.decode_all(z=z_kp, z_features=z_features, z_bg_features=z_bg, obj_on_sample=z_obj_ons,
                                             z_depth=z_depths, z_scale=z_scales,
                                             z_ctx=z_context)
        return decoder_dict
    ...
    def update_image(self):
        ...
        decoder_dict = self.decode_particles(updated_coordinates, updated_scales, self.original_depths, updated_obj_ons,
                                             updated_features, self.original_bg, self.original_ctxs)
        self.img = ToPILImage()(decoder_dict['rec_rgb'][0].clamp(0, 1).cpu())
        ...
        if self.model_type == 'dlp':
            ...
            alpha, rgb = torch.split(glimpses, [1, glimpses.shape[1] - 1], dim=1)
            rgb = rgb[:, :3]
            ...
            alpha, rgb = torch.split(glimpses, [1, glimpses.shape[1] - 1], dim=2)
            rgb = rgb[:, :, :3]
            rgba = alpha * rgb  # [T, n_particles, 3, h, w]
            rgba = rgba.clamp(0, 1).permute(1, 0, 2, 3, 4).cpu()  # [n_particles, T, 3, h, w]
            glimpses = []
    ...
    def unroll_particles(self, num_steps=60):
        ...
        # [T, F]
        z_context = torch.tensor(ctx,
                                 device=torch.device(self.device_name), dtype=torch.float).reshape(-1, self.n_frames,
                                                                                                   self.original_ctxs.shape[
                                                                                                       -1])
        # Generate the sequence of images based on the current particles
        # dynamics
        dyn_out = self.model.dyn_module.sample(z_kp, z_scales, z_obj_ons, z_depths, z_features, z_bg, z_context,
                                               steps=num_steps, deterministic=True)
        z_dyn, z_scale_dyn, z_obj_on_dyn, z_depth_dyn, z_features_dyn, z_bg_features_dyn, z_context_dyn = dyn_out
        # decode
        ...
        z_context_dyn = z_context_dyn[:, 1:].reshape(-1, *z_context_dyn.shape[2:])
        ...
        dec_out = self.model.decode_all(z_dyn, z_features_dyn, z_bg_features_dyn, z_obj_on_dyn,
                                        z_depth=z_depth_dyn, z_scale=z_scale_dyn, z_ctx=z_context_dyn)
        # rec_dyn = dec_out['rec'].clamp(0, 1)
        rec_dyn = dec_out['rec_rgb'].clamp(0, 1)
    ...
    def update_image_t(self):
        ...
        for k in range(len(inter_steps)):
            ...
            imgs.append(ToPILImage()(decoder_dict['rec_rgb'][0].clamp(0, 1).cpu()))
