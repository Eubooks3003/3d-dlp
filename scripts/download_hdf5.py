...
def extract_trajectory(
    ...
    robot0_gripper_pcd = [get_4_points_from_gripper_pos_orient(eef_pos, eef_quat, eef_qpos[0]) for eef_pos, eef_quat, eef_qpos 
                 in zip(robot0_eef_pos, robot0_eef_quat, robot0_gripper_qpos)]
    robot0_gripper_pcd = np.array(robot0_gripper_pcd)
    goal_gripper_pcd, switch_idxs = compute_new_goal_gripper_pcd(robot0_gripper_pcd, 
                                                                 robot0_gripper_qpos,
                                                                 actions,
    ...
    traj['unique_goal_gripper_pcds'] = goal_gripper_pcd[np.asarray(switch_idxs)-1]
    # list to numpy array
    for k in traj:
...
def worker(x):
    env_meta, args, initial_state, states, actions, robot0_eef_pos, robot0_eef_quat, robot0_gripper_qpos = x
...
def dataset_states_to_obs(args):
    ...
    for i in range(0, len(demos), num_workers):
        ...
        for j in range(i, end):
            ...
            robot0_eef_quat_list.append(robot0_eef_quat)
            robot0_gripper_qpos_list.append(robot0_gripper_qpos)

        with multiprocessing.Pool(num_workers) as pool:
            trajs = pool.map(worker, [[env_meta, args, 
                                       initial_state_list[j],
                                       states_list[j],
                                       actions_list[j],
                                       robot0_eef_pos_list[j],
                                       robot0_eef_quat_list[j],
                                       robot0_gripper_qpos_list[j]] for j in range(len(initial_state_list))]) 
        ...
        for j, ind in enumerate(range(i, end)):
            ...
            ep_data_grp.create_dataset("unique_goal_gripper_pcds", data=np.array(traj["unique_goal_gripper_pcds"]))
