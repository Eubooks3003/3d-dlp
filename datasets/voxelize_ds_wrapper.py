...
class VoxelizedDataset(Dataset):
    ...
    it precomputes voxels once and reads them lazily later.
    ...
    def __init__(self,
       ...
       # --- inside VoxelizedDataset.__init__ ---
        ...
        # --------------------------
        # BOUNDS SELECTION (simplified)
        # --------------------------
        self.bounds = None
        if bounds_mode == "global":
            # If manifest exists, ADOPT its bounds and skip scanning.
            if mani is not None and "pmin" in mani and "pmax" in mani:
                ...
                # First build: compute once, then it will be reused from manifest next runs
        ...
        # --------------------------
        # CACHE DECISION (simple)
        # --------------------------
        self._use_cache = False
        if self.cache_dir:
            os.makedirs(self.cache_dir, exist_ok=True)
            # if manifest present, just check basic knobs + item files
            if (not force_rebuild) and (mani is not None):
                ...
                # bounds already adopted above when bmode is global
                if cache_ok and self._all_item_files_exist():
                    self._use_cache = True

        # If no usable cache → build and write manifest (with the bounds we picked)
        if not self._use_cache:
            self._build_and_optionally_cache(bmode_str)
        else:
            # lazy path: files will be read in __getitem__
            pass
    ...
    def _build_and_optionally_cache(self, bmode_str: str):
        ...
        for i in tqdm(range(len(self.base_ds)), desc="Voxelizing dataset"):
            ...
            vox = vg.to_dense().to(self.device)         # [C,D,H,W] on target device
            md  = vg.meta_dict()
            # move meta tensors to device
            ...
            if self.cache_dir:
                ...
                torch.save(vox.cpu(), vp)             # store CPU for portability
                # store meta on CPU too
                md_cpu = {k: (v.cpu() if torch.is_tensor(v) else v) for k,v in md.items()}
                torch.save(md_cpu, mp)
                if extra:
                    try:
                        # be defensive with extras; they may contain non-serializable stuff
                        extra_cpu = {}
    ...
    def __getitem__(self, idx: int) -> Dict[str, Any]:
        ...
        if self.cache_dir and self._use_cache and not hasattr(self, "_voxels"):
            ...
            # 1) load extras (including cached points)
            extras = {}
            if ep and os.path.isfile(ep):
                extras = torch.load(ep, map_location="cpu")
            # optional: merge with current base_ds metadata (id/path), but don't override points
