...
DATASET_NAME = 'bridge'
OUTPUT_DIR = '/data/bridge'  # Directory to save the preprocessed data
...
dataset_directories = {
    ...
    'bridge': 'gs://gresearch/robotics/bridge'
}
...
def preprocess_instruction(instruction_str, tokenizer, max_length):
    ...
    encoded_instruction = tokenizer(instruction_str, max_length=max_length, padding='max_length', truncation=True,
                                    return_tensors='pt')
    return encoded_instruction['input_ids'].squeeze(0)  # Return a 1D tensor


def save_episode(episode, split, episode_id, output_dir, tokenizer, max_instruction_length, resize_resolution,
                 t5_encoder):
    """Saves the frames, metadata (as .pt), and processed instruction (as .pt)."""
    episode_dir = os.path.join(output_dir, split, str(episode_id))
    ...
    for i, step in tqdm(enumerate(steps), total=len(steps_dataset), desc=f"Saving data for {episode_id}"):
        # Save image
        img = Image.fromarray(step['observation']['image'])
        ...
        # Save metadata as .pt
        metadata = {
            'action': torch.tensor(step['action']['rotation_delta'], dtype=torch.float32),
            'action_gripper': torch.tensor(step['action']['open_gripper'], dtype=torch.bool),
            'action_world_vector': torch.tensor(step['action']['world_vector'], dtype=torch.float32),
            ...
            'state': torch.tensor(step['observation']['state'], dtype=torch.float32)
        }
        ...
        if i == 0:
            ...
            instruction_str = step['observation']['natural_language_instruction'].decode("utf-8")
            processed_instruction = preprocess_instruction(instruction_str, tokenizer, max_instruction_length)
            instruction_embedding_t5 = t5_encoder(processed_instruction.unsqueeze(0)).last_hidden_state.squeeze(0)
            instruction_embedding = torch.tensor(step['observation']['natural_language_embedding'], dtype=torch.float32)
            instruction_data = {'raw_instruction': instruction_str, 'processed_instruction': processed_instruction,
                                'instruction_embedding_t5': instruction_embedding_t5,
                                'instruction_embedding': instruction_embedding}
            instruction_path = os.path.join(episode_dir, 'instruction.pt')
            torch.save(instruction_data, instruction_path)


def process_and_split(dataset, output_dir, tokenizer, max_instruction_length, resize_resolution, valid_ratio,
                      test_ratio, random_seed, t5_encoder):
    ...
    # episode_items = list(dataset.as_numpy_iterator())
    # episode_items = iter(dataset)
    # episode_ids_with_data = {item['episode_id'].decode(): item for item in episode_items}
    # episode_ids_with_data = {item['episode_id'].numpy().decode(): item for item in episode_items}
    n_ep = 0
    # episode_ids_with_data = {}
    all_episode_ids = []
    for i, ep in tqdm(enumerate(dataset)):
        ...
        n_ep += 1
        save_episode(ep, 'train', ep_id, output_dir, tokenizer, max_instruction_length, resize_resolution, t5_encoder)
        # episode_ids_with_data [item['episode_id'].numpy().decode()] = item
    # all_episode_ids = list(episode_ids_with_data.keys())
    ...
    # print(f"Processing training split ({len(train_episode_ids)} episodes)...")
    # for episode_id in tqdm(train_episode_ids, desc="Processing train episodes"):
    #    save_episode(episode_ids_with_data[episode_id], 'train', episode_id, output_dir, tokenizer, max_instruction_length, resize_resolution, t5_encoder)

    # print(f"Processing validation split ({len(valid_episode_ids)} episodes)...")
    # for episode_id in tqdm(valid_episode_ids, desc="Processing validation episodes"):
    #    save_episode(episode_ids_with_data[episode_id], 'valid', episode_id, output_dir, tokenizer, max_instruction_length, resize_resolution, t5_encoder)

    # print(f"Processing test split ({len(test_episode_ids)} episodes)...")
    # for episode_id in tqdm(test_episode_ids, desc="Processing test episodes"):
    #    save_episode(episode_ids_with_data[episode_id], 'test', episode_id, output_dir, tokenizer, max_instruction_length, resize_resolution, t5_encoder)

    move_episodes(output_dir, valid_episode_ids, test_episode_ids)

    # Optionally, you might want to verify the number of episodes in each directory
    # print(f"\nNumber of episodes in train directory: {len([d for d in os.listdir(train_dir) if os.path.isdir(os.path.join(train_dir, d))])}")
    # print(f"Number of episodes in valid directory: {len([d for d in os.listdir(os.path.join(output_dir, 'valid')) if os.path.isdir(os.path.join(output_dir, 'valid'), d)])}")
    # print(f"Number of episodes in test directory: {len([d for d in os.listdir(os.path.join(output_dir, 'test')) if os.path.isdir(os.path.join(output_dir, 'test'), d)])}")
...
if __name__ == '__main__':
    ...
    process_and_split(full_dataset, OUTPUT_DIR, tokenizer, MAX_INSTRUCTION_LENGTH, RESIZE_RESOLUTION, VALID_RATIO,
                      TEST_RATIO, RANDOM_SEED, t5_encoder)
