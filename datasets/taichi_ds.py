"""
Dataset class for the TaiChi Dataset
"""
...
class TaiChiDataset(Dataset):
    def __init__(self, root, mode, ep_len=1024, sample_length=10, image_size=128, dense=False, clips_per_video=40,
        ...
        elif self.mode == 'test' and self.n_max_valid > 0:
        ...
        if self.mode == 'train':
            ...
            for pi, path in enumerate(paths):
                ...
                done_i = torch.tensor((pi < ep_len),
                                      dtype=torch.int)  # episode_mask: 1 if valid else 0, after end of episode
                dones.append(done_i)
                if len(imgs) == self.sample_length:
                    break
...
class TaiChiDatasetImage(Dataset):
    def __init__(self, root, mode, ep_len=1024, sample_length=1, image_size=128, force_cache_rebuild=False):
        assert mode in ['train', 'val', 'valid', 'validation', 'test']
        if mode in ["val", "valid", "validation" "test"]:
            mode = 'test'
        self.mode = mode
...
if __name__ == '__main__':
    ...
    plot = False
    # --- episodic setting --- #
    root = '/data/taichi/Taichi-HD/taichi-256/frames'
    ds = TaiChiDataset(root=root, ep_len=1024, sample_length=12, mode='train', image_size=128, dense=True)
