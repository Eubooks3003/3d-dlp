...
class LanguageTableDatasetImage(Dataset):
    def __init__(self, root, mode, ep_len=45, sample_length=15, image_size=128):
...
if __name__ == '__main__':
    ...
    # --- episodic setting --- #
    root = '/media/newhd/data/lang_table/proc_data'
    # ds = LanguageTableDataset(root=root, ep_len=45, sample_length=15, mode='train', image_size=128,
    #                           normalize_embeddings=norm_embeddings)
    # ds = LanguageTableDataset(root=root, ep_len=45, sample_length=15, mode='valid', image_size=128)
    ds = LanguageTableDataset(root=root, ep_len=45, sample_length=15, mode='test', image_size=128)
    dl = DataLoader(ds, shuffle=True, pin_memory=False, batch_size=3, num_workers=4)
    batch = next(iter(dl))
    im, text_embeddings, instructions, actions, rewards, ind = batch
    # im = batch[0]
